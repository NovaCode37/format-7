...
type Summary = {
  format: string; method: string; storage: string;
  quantity: number; total: number; pagePrice: number;
};
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const options = {
        ...
        storage: summary.storage === "Да" ? "На наш носитель (+600₽)" : "Email / носитель клиента",
        pages: summary.quantity,
      };

      const fullComment = [
        `Сканирование: ${summary.format}, ${summary.method.toLowerCase()}`,
        `${summary.quantity} стр.`,
        `Носитель: ${summary.storage === "Да" ? "наш (+600₽)" : "email/клиента"}`,
        `Итого: ${summary.total} ₽`,
        comment || "",
      ].filter(Boolean).join(" | ");
        ...
        {step === "form" ? (
          <form onSubmit={submit}>
            ...
            <div className="mt-4 rounded-lg bg-ink-50 border border-ink-200 p-3 text-[12px] text-ink-700 space-y-0.5">
              <p>Сканирование · {summary.format} · {summary.method}</p>
              <p>{summary.quantity} страниц</p>
              <p>Носитель: {summary.storage === "Да" ? "наш (+600₽)" : "email / носитель клиента"}</p>
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-ink-600 text-center">
              ...
              {email ? <> или email {email}</> : null} для подтверждения заказа.
            </p>
            {!!orderNumber && (

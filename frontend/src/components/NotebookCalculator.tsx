...
type BlockColor = "Без печати" | "Чёрно-белая (1+0)" | "Цветная (4+0)";
...
const PRICE: Record<Format, Record<BlockColor, number>> = {
  "А6 (105×148 мм)": { "Без печати": 110, "Чёрно-белая (1+0)": 180, "Цветная (4+0)": 250 },
  "А5 (148×210 мм)": { "Без печати": 160, "Чёрно-белая (1+0)": 225, "Цветная (4+0)": 370 },
  "А4 (210×297 мм)": { "Без печати": 250, "Чёрно-белая (1+0)": 285, "Цветная (4+0)": 430 },
};
...
export default function NotebookCalculator({ serviceId }: { serviceId?: number }) {
  ...
  const [coverSides, setCoverSides] = useState<Sides>("Односторонняя");
  const [blockColor, setBlockColor] = useState<BlockColor>("Без печати");
  const [blockSides, setBlockSides] = useState<Sides>("Односторонняя");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5">
            <div className="rounded-xl border border-ink-200 bg-white p-5 sm:p-6 space-y-5">
              ...
              <PillsField label="Цветность блока" values={["Без печати", "Чёрно-белая (1+0)", "Цветная (4+0)"]} value={blockColor} onChange={(v) => setBlockColor(v as BlockColor)} />
              <PillsField label="Стороны печати обложки и подложки" values={["Односторонняя", "Двусторонняя"]} value={coverSides} onChange={(v) => setCoverSides(v as Sides)} hint="на цену не влияет" />
              <PillsField label="Стороны печати блока" values={["Односторонняя", "Двусторонняя"]} value={blockSides} onChange={(v) => setBlockSides(v as Sides)} hint="на цену не влияет" />
              ...
              <div className="pt-4 border-t border-ink-100">
                <QuantityField presets={QTY_PRESETS} value={quantity} onChange={setQuantity} min={1} hint="Цена указана за 1 шт при тираже 1–100." />
              </div>
              <div className="pt-4 border-t border-ink-100">
